...
class GetDashboardUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<DashboardData> = combine(
        ...
        backupRepository.observeBackupMetadata()
    ) { cards, tags, folders, collections, activities, backup ->
        DashboardData(
            stats = VaultStats(cards.size, cards.count { it.isPinned }, cards.count { it.isFavorite }, tags.size, folders.size, collections.size, cards.sumOf { it.attachments.size }, backup?.lastExportedAt),
            recentCards = cards.sortedByDescending { it.updatedAt }.take(8),
            pinnedCards = cards.filter { it.isPinned }.take(8),
            topTags = tags.sortedByDescending { it.useCount }.take(12),
            activeCollections = collections.sortedByDescending { it.cardCount }.take(6),
