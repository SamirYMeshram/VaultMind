...
@Dao interface TagDao {
    @Query("SELECT tags.*, COUNT(card_tag_cross_refs.card_id) AS useCount FROM tags LEFT JOIN card_tag_cross_refs ON tags.id = card_tag_cross_refs.tag_id GROUP BY tags.id ORDER BY useCount DESC, name ASC")
...
@Dao interface CollectionDao {
    @Query("SELECT collections.*, COUNT(card_collection_cross_refs.card_id) AS cardCount FROM collections LEFT JOIN card_collection_cross_refs ON collections.id = card_collection_cross_refs.collection_id GROUP BY collections.id ORDER BY name ASC")
